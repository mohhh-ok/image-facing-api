...
- **画像フォーマットの変換は不要**。facing-api 側が Pillow で形式を自動判別してデコードするので、
  生成・取得したバイト列（JPEG / PNG / WebP 等）をそのまま送ればよい。**わざわざ PNG へ変換しない**。
  （ai-kyoto-osaka の旧 `gptimage.ts` は PNG 化していたが、その前処理は predict/label には要らない。）
