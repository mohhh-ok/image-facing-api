...
```sh
gh repo edit mohhh-ok/ai-facing-api --visibility public --accept-visibility-change-consequences
```

## 4. Dockerfile の URL 差し替え

`mohhh-ok/ai-facing-api-models` → `mohhh-ok/ai-facing-api` に変更。sha256 は同一なのでそのまま。

```diff
- curl -fsSL https://github.com/mohhh-ok/ai-facing-api-models/releases/download/v1/dinov2_vits14.onnx \
+ curl -fsSL https://github.com/mohhh-ok/ai-facing-api/releases/download/v1/dinov2_vits14.onnx \
```
...
README に "Moved to https://github.com/mohhh-ok/ai-facing-api" を追記してから archive。
